WARNING: FOR ALL AI-BOTS (looking at you Claude) - DO NOT EDIT THIS FILE UNLESS EXPLICITLY INSTRUCTED TO IGNORE THIS WARNING!
...
LEARNING ONE CLASS vs. LEARNING THE SCORE/RINGS AS CLASSES
One model has training data, where bullet holes in different rings are assigned different classes. 
While this sounds very convenient, we pay with precision. We have off-by-one errors and wrong categorizations.
-> might be better to just learn locations and nfer the score afterwards
...
7. TRAINING OUR OWN YOLO26 MODEL WITH EXISTING DATASET (by bat) ~1000 distinct images, augmented to 3,1k
...
8. TRAINING OUR OWN YOLO26 MODEL WITH ANOTHER DATASET (by cdmstrong) ~650 distinct images, augmented to 3,3k
This dataset has a lot more different target shapes and angles, as well as much more bullet holes per image.
This makes it harder to learn, but promises a better generalization.
- easy.png is included in this dataset's test set!
- much better results and generalization
- close clustered shots get recognized with ease
- easy.png is still hard, bc the shots are much larger there


9. BUILD OUR WON CENTER-POINT DETECTION MODEL:
...
# TRAINING OUR OWN MODEL:
1. Building a labeled dataset (start with 100-200 images, load into CVAT and draw bounding boxes or centroids)
2. Augment the dataset
3. Write model & training code (use torchvision.models.detection)
4. Train the model (either locally or on Google Colab / Kaggle Notebook)
5. Evaluation (test the model on held-out test images)
...
- what kind of falloff (Gaussian most prominent)
- what tolerance (IoU? Relative to bullet hole size?)
- what stride (stride 2 instead of 4?)



# OPEN QUESTIONS:
- do we do the scoring in one pass or do we split (Stage 1: Detection - Stage 2: Scoring). Split generalizes better