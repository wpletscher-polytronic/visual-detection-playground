...
from centerpoint.metrics import (CENTRE_TOLERANCES_PX, PRIMARY_CENTRE_TOLERANCE_PX,
                                 RECALL_POINTS, assign_by_confidence, assign_optimal,
                                 average_precision, centre_criterion, circle_iou,
...
def test_average_precision_agrees_with_the_slow_reference(seed):
    ...
    labels = [(float(rng.choice([0.95, 0.8, 0.8, 0.5, 0.2, 0.05])), bool(rng.integers(0, 2)))
...
def test_the_metric_names_state_criterion_threshold_and_rule():
    results = evaluate_split([(holes((10.0, 10.0, 3.0)), detections((10.0, 10.0, 3.0, 0.9)))])
...
def test_the_contract_metadata_records_the_policy():
    ...
    assert contract['operating_threshold_status'].startswith('provisional')
    assert contract['candidate_floor'] < contract['operating_threshold']
    assert contract['recall_points'] == 101
    assert contract['detection_cap_bound'] is False
...
def test_a_perfect_split_scores_one_everywhere():
    ...
    results = evaluate_split(samples)
...
def test_a_split_of_empty_images_reports_undefined_rather_than_perfect():
    results = evaluate_split([(holes(), detections()), (holes(), detections())])
